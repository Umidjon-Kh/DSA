...
| Class                   | Resizable | Type-safe | Backed by              |
| ----------------------- | --------- | --------- | ---------------------- |
| `StaticUniversalArray`  | ✗         | ✗         | `ctypes.py_object`     |
| `StaticTypedArray`      | ✗         | ✓         | raw `ctypes` buffer    |
| `DynamicUniversalArray` | ✓         | ✗         | `StaticUniversalArray` |
| `DynamicTypedArray`     | ✓         | ✓         | `StaticTypedArray`     |
...
Enforces a single dtype — lower memory overhead than universal.
...
Dynamic arrays grow using the same formula as CPython's built-in `list`:
...
When `capacity` is omitted, it is inferred from the number of initial elements.
At least one of `capacity` or `*args` must be present — passing neither raises `TypeError`.
...
so strings work out of the box without extra configuration.
