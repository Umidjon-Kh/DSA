...
class BaseStaticArray(BaseArray):
    """
    Abstract base class for fixed-size arrays.

    Extends BaseArray with capacity tracking and a fullness check.
    The buffer size is set at construction and never changes.

    Required to implement (in addition to BaseArray):
        capacity (property), is_full
    """

    @property
    @abstractmethod
    def capacity(self) -> int:
        """Returns the fixed maximum number of elements the array can hold."""
        ...
