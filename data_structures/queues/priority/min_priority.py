...
from ...heaps import DynamicUniversalMinHeap
...
class MinPriorityQueue(BasePriorityQueue):
    ...
    Backed by DynamicUniversalMinHeap. Wrapping into PriorityObject is
    handled internally — the caller simply passes a value and a priority.
    The heap orders elements via PriorityObject.__lt__ so the root is
    always the object with the smallest priority number.

    ── Ordering ─────────────────────────────────────────────────────────────
    Lower priority number = dequeued first.

        q.enqueue("low urgency",  priority=10)
        q.enqueue("high urgency", priority=1)
        q.dequeue()  ->  "high urgency"   # priority=1 comes out first

    ── Backed by heap ────────────────────────────────────────────────────────
    Internally every value is wrapped in a PriorityObject and pushed onto
    a DynamicUniversalMinHeap. The heap grows automatically — no capacity
    ...
    def __init__(self, *args: tuple) -> None:
        """
        Creates a min priority queue with optional initial elements.
        ...
        """
        self._heap: DynamicUniversalMinHeap = DynamicUniversalMinHeap()
    ...
    def enqueue(self, value: Any, priority: int | float) -> None:
        ...
        onto the heap. The heap restores the min-heap property via
    ...
    def dequeue(self) -> Any:
        """
        Removes and returns the value of the element with the lowest
    ...
    def peek(self) -> Any:
        """
        Returns the value of the element with the lowest priority number
    ...
    def copy(self) -> "MinPriorityQueue":
        """
        ...
        heap into a new MinPriorityQueue — heap property is preserved.

        Time complexity: O(n log n)
        """
        new_queue = MinPriorityQueue()
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        if not isinstance(other, MinPriorityQueue):
    ...
    def __repr__(self) -> str:
        ...
        Format: MinPriorityQueue(size=2)[(value='fix bug', priority=1), ...]
                                         root (lowest priority number)
        ...
        return f"MinPriorityQueue(size={len(self._heap)})[{elements}]"
