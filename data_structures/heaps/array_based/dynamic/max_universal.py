...
class DynamicUniversalMaxHeap(BaseHeap):
    """
    A resizable max-heap backed by DynamicUniversalArray.
    ...
    def __init__(self, *args) -> None:
        """
        Creates a resizable universal max-heap with optional initial elements.
        ...
        Examples:
            h = DynamicUniversalMaxHeap()          # empty
            h = DynamicUniversalMaxHeap(1, 3, 5)   # root=5
        """
    ...
    def copy(self) -> "DynamicUniversalMaxHeap":
        """
        Returns a shallow copy with the same elements.

        Time complexity: O(n)
        """
        new_heap = DynamicUniversalMaxHeap()
    ...
    def __eq__(self, other: object) -> bool:
        ...
        """
        if not isinstance(other, DynamicUniversalMaxHeap):
    ...
    def __repr__(self) -> str:
        ...
        Format: DynamicUniversalMaxHeap(size=3)[5, 3, 1]
                                                 root

        Time complexity: O(n)
        """
        elements = ", ".join(repr(self._data[i]) for i in range(len(self._data)))
        return f"DynamicUniversalMaxHeap(size={len(self._data)})[{elements}]"
