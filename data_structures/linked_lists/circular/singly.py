...
class CircularSinglyLinkedList(BaseLinkedList):
    """
    A circular singly linked list backed by LinearNode.
    ...
    def __init__(self, *args) -> None:
        ...
        self._tail: Optional[LinearNode] = None
        self._size: int = 0
    ...
    def _get_node(self, index: int) -> LinearNode:
        """
    ...
    def _append_to_empty(self, node: LinearNode) -> None:
        """
    ...
    def __reversed__(self) -> Iterator[Any]:
        ...
        current = self._head
        for _ in range(self._size):
            temp = LinearNode(current.value)  # type: ignore[union-attr]
            temp.next = temp_head
            temp_head = temp
            current = current.next  # type: ignore[union-attr]
        current = temp_head
