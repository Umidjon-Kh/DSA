...
from ..._base import BaseLinkedList
...
class DoublyLinkedList(BaseLinkedList):
    """
    A doubly linked list backed by BiLinearNode.
    Each node holds a value and references to both next and previous nodes.
    ...
    def __init__(self, *args) -> None:
        ...
        """
        self._head: Optional[BiLinearNode] = None
        self._tail: Optional[BiLinearNode] = None
        self._size: int = 0
    ...
    def _get_node(self, index: int) -> BiLinearNode:
        """
    ...
    def _append_to_empty(self, node: BiLinearNode) -> None:
        """
    ...
    def _link_nodes(self, a: BiLinearNode, b: BiLinearNode) -> None:
        """
    ...
    def _unlink_node(self, node: BiLinearNode) -> None:
        """
    ...
    def remove(self, index: int) -> Any:
        ...
        if self._size == 1:
            ...
            self._head.prev = None  # type: ignore[union-attr]
        elif target is self._tail:
            self._tail = self._tail.prev  # type: ignore[union-attr]
            self._tail.next = None  # type: ignore[union-attr]
        else:
    ...
    def __repr__(self) -> str:
        ...
        Format: DoublyLinkedList(size=3)[1 <-> 2 <-> 3]

        Time complexity: O(n)
        """
        elements = " <-> ".join(repr(v) for v in self)
        return f"DoublyLinkedList(size={self._size})[{elements}]"
